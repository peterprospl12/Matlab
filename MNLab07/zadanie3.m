function [integration_error, Nt, ft_5, integral_1000] = zadanie3()
    ...
    ft_5 = fun(5);
    integral_1000 = simpson_method(@fun, 1000, 0, 5);
    
    integration_result = zeros(1, length(Nt));

    for i = 1:length(Nt)
        integration_result(1,i) = simpson_method(@fun, Nt(i), 0, 5);
    ...
    title('Integration error for simpson method');
    legend('show');
...
function integral = simpson_method(funHandle, N, a, b)
    ...
    for i = 1:N
        ...
        sec_value = a + i * delta;
        fst_sec_value = funHandle((first_value+ sec_value)/2);
        value = (funHandle(first_value) + 4*fst_sec_value + funHandle(sec_value));
        integral = integral + value;
    end
    integral = integral * (delta / 6);
