function [integration_error, Nt, ft_5, integral_1000] = zadanie1()
    ...
    ft_5 = fun(5);
    integral_1000 = rectangular_method(@fun, 1000, 0, 5);
    
    integration_result = zeros(1, length(Nt));

    for i = 1:length(Nt)
        integration_result(1,i) = rectangular_method(@fun, Nt(i), 0, 5);
    ...
    title('Integration error for rectangular method');
    legend('show');
...
function integral = rectangular_method(funHandle, N, a, b)
    ...
    for i = 1:N
        ...
        sec_value = a + i * delta;
        value = funHandle((first_value + sec_value) / 2);
