...
if isfield(energy, country) && isfield(energy.(country), source)
    ...
    for i=1:length(y_yearly)
        X = dct2_custom(y_yearly, i);
        temp{i} = idct2_custom(X, i, N, P);
        mse(i, 1) = mean((y_yearly - idct2_custom(X, i, N, N)).^2);
         if i > 1
            msek(i-1,1) = mean((temp{i-1} - temp{i}).^2);
         end
    end
    ...
    subplot(3,1,2);
    semilogy(1:length(y_yearly), mse, 'b', 'DisplayName', sprintf("MSE"));
    ...
    subplot(3,1,3);
    semilogy(1:length(y_yearly)-1, msek, 'b', 'DisplayName', sprintf("MSEk"));
    xlabel('Stopień wielomianu');
